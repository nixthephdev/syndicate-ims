...
function Wordmark() {
    return (
        <Link href="/" className="group flex items-center gap-3">
            {/* White-on-transparent mark — only safe on dark surfaces. On a
                light background use logo-mark-dark.png instead. */}
            <img
                src="/images/logo-mark.png"
                alt=""
                aria-hidden="true"
                className="h-8 w-auto shrink-0 transition-transform duration-300 group-hover:-rotate-6 sm:h-10"
            />
            <span className="flex flex-col leading-none">
                <span className="font-display text-2xl uppercase leading-none tracking-tight text-white transition-colors group-hover:text-volt-500 sm:text-3xl">
                    Syndicate
                </span>
                <span className="mt-1 hidden text-[10px] font-bold uppercase tracking-[0.35em] text-white/40 sm:inline">
                    Supply Co.
                </span>
            </span>
        </Link>
...
export default function StoreHeader() {
    ...
    const navLink = (item) => {
        if (!route().has(item.route)) {
            return (
                ...
                >
                    {item.label}
                    <sup className="ml-1 text-[9px] tracking-normal text-volt-500/60">
                        soon
                    </sup>
                </span>
            );
        ...
        return (
            <Link
                ...
                className={`${linkBase} ${
                    active
                        ? 'text-volt-500'
                        : 'text-white hover:text-volt-500'
                }`}
            >
                {item.label}
            </Link>
    ...
    return (
        <header className="sticky top-0 z-50 border-b-2 border-volt-500 bg-ink-950/95 backdrop-blur supports-[backdrop-filter]:bg-ink-950/80">
            <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:h-20 sm:px-6 lg:px-8">
                ...
                <div className="flex items-center gap-4 sm:gap-6">
                    {/* Auth — real routes, these always exist (Breeze). */}
                        ...
                        {user ? (
                            <>
                                ...
                                <Link
                                    ...
                                    method="post"
                                    as="button"
                                    className={`${linkBase} text-white/50 hover:text-white`}
                                >
                                    Log out
                                </Link>
                            </>
                            ...
                            <>
                                ...
                                <Link
                                    href={route('register')}
                                    className="bg-volt-500 px-4 py-2 font-display text-sm uppercase tracking-[0.2em] text-ink-900 transition-transform hover:-translate-y-0.5 hover:bg-white"
                                >
                                    Sign up
                                </Link>
                            </>
                    ...
                    {cartHref ? (
                        <Link
                            ...
                            className="text-white transition-colors hover:text-volt-500"
                        >
                            {cartButton}
                        </Link>
                    ) : (
                        ...
                        {user ? (
                            <>
                                ...
                                <Link
                                    ...
                                    method="post"
                                    as="button"
                                    className={`${linkBase} text-left text-white/50`}
                                >
                                    Log out
                                </Link>
                            </>
